...
public class AcceleratedRectangleRenderer implements IAcceleratedRenderer<RectangleDrawContext> {
	...
	public void render(
		...
		vertexConsumer.vertex(minX, minY, 0).color(color);
		vertexConsumer.vertex(minX, maxY, 0).color(color);
		vertexConsumer.vertex(maxX, maxY, 0).color(color);
		vertexConsumer.vertex(maxX, minY, 0).color(color);
